...
D3DApp::D3DApp(HINSTANCE hInstance)
{
	...
	mClearColor     = D3DXCOLOR(0.2f, 0.5f, 0.9f, 1.0f);
	mClientWidth    = 1024;
	mClientHeight   = 768;
}
...
void D3DApp::initDirect3D()
{
	...
	HRESULT myHR =  D3D10CreateDeviceAndSwapChain(
			0,                 //default adapter
			md3dDriverType,
	...
	switch (myHR){
		case E_INVALIDARG:
			OutputDebugString(L"An invalid parameter was passed to the returning function");
			MessageBox(0, L"Program Failed", 0, 0);
			PostQuitMessage(0);
			break;
	}
	HR(myHR);
